    ...
    async def on_message(self, message):
        ...
        if data[str(message.guild.id)]["malblock"] == True:
            ...
            for x in range(len(s)):
                if "roblox" in s[x]:
                    embed = discord.Embed(title=":warning: Compromised Link Detected", description=f"Looks like {message.author.mention} **({message.author.id})** sent an **Compromised Roblox Link** which could get your roblox account vunerable.\n\n**Link:** ||[{s[x]}](https://www.youtube.com/watch?v=bzXzGMbdQfY)||\n**Note: This link won't direct you to the compromised site to avoid users visiting the site.**")
                    try:
                        requests.get(s[x], timeout=2)
                    except:
                        return
