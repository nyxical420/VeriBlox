from DataTools import appendUserData, readUserData, appendGuildData, refreshUserData
...
class usercommands(commands.Cog):
    ...
    async def whois(self, interaction : Interaction, moreinfo : str, member : Optional[Member] = None):
        ...
        profile_button = Button(label="Visit Roblox Profile", style=ButtonStyle.url, url=f"https://www.roblox.com/users/{uid}/profile")
        avatarimage = get(f"https://thumbnails.roblox.com/v1/users/avatar?userIds={uid}&size=420x420&format=Png&isCircular=false").json()
        delete = Button(label="Delete", style=ButtonStyle.red)
        ...
        if info["banned"] == True:
            embed=Embed(title=f"{member.name}'s Roblox Profile", description=f"**Banned**\n{description}", color=0xFF6961)
        else:
            if s.startswith("Online"):
                embed=Embed(title=f"{member.name}'s Roblox Profile", description=f"**Online**\n{description}", color=0x00A2FF)
            if s.startswith("Playing"):
                embed=Embed(title=f"{member.name}'s Roblox Profile", description=f"**Playing**\n{description}", color=0x02B757)
            if s.startswith("Creating"):
                embed=Embed(title=f"{member.name}'s Roblox Profile", description=f"**Creating (On Studio)**\n{description}", color=0xF68802)
            if s.startswith("Offline"):
                embed=Embed(title=f"{member.name}'s Roblox Profile", description=f"**Offline**\n{description}", color=0x202225)
    ...
    @app_commands.checks.cooldown(1, 5, key=lambda i: i.user.id)
    async def avatar(self, interaction : Interaction, member : Optional[Member], viewtype : str, imageres : int):
        member = interaction.user if not member else member
        member_id = str(member.id)
        data = readUserData()
        ...
        uid = data[member_id]["robloxid"]
        if imageres == 0:
            embed = Embed(title=f"{member}'s Roblox Avatar (150 x 150)")
        if imageres == 1:
            embed = Embed(title=f"{member}'s Roblox Avatar (180 x 180)")
        if imageres == 2:
            embed = Embed(title=f"{member}'s Roblox Avatar (352 x 352)")
        if imageres == 3:
            embed = Embed(title=f"{member}'s Roblox Avatar (420 x 420)")
        if imageres == 4:
            embed = Embed(title=f"{member}'s Roblox Avatar (720 x 720)")
    ...
    async def search(self, interaction : Interaction, search_type : str, search : str, moreinfo : str):
        ...
        if search_type == "username":
            ...
            if info["banned"] == True:
                embed=Embed(title=info["username"] + f"'s Roblox Profile", description=f"**Banned**\n{description}", color=0xFF6961)
            else:
                if s.startswith("Online"):
                    embed=Embed(title=info["username"] + f"'s Roblox Profile", description=f"**Online**\n{description}", color=0x00A2FF)
                if s.startswith("Playing"):
                    embed=Embed(title=info["username"] + f"'s Roblox Profile", description=f"**Playing**\n{description}", color=0x02B757)
                if s.startswith("Creating"):
                    embed=Embed(title=info["username"] + f"'s Roblox Profile", description=f"**Creating (On Studio)**\n{description}", color=0xF68802)
                if s.startswith("Offline"):
                    embed=Embed(title=info["username"] + f"'s Roblox Profile", description=f"**Offline**\n{description}", color=0x202225)
                view.add_item(profile_button)
        ...
        if search_type == "userid":
            ...
            profile_button = Button(label="Visit Roblox Profile", style=ButtonStyle.url, url=f"https://www.roblox.com/users/{int(search)}/profile")
            try:
    ...
    async def invite(self, interaction : Interaction):
        ...
        vb_invite  = Button(label="Invite VeriBlox", style=ButtonStyle.url, url="https://discord.com/api/oauth2/authorize?client_id=872081372162973736&permissions=1377007119382&scope=bot%20applications.commands")
        vb_support = Button(label="VeriBlox Support Server", style=ButtonStyle.url, url="https://discord.gg/EHNtECJRKA")
        vb_topgg   = Button(label="VeriBlox Top.gg", style=ButtonStyle.url, url="https://top.gg/bot/872081372162973736")

        embed = Embed(title="VeriBlox Invite", description="Click the **Invite VeriBlox** button below to invite VeriBlox to your server!\n\nIf you want to support the development of **VeriBlox**, visit Veriblox's Top.gg page below by clicking the **VeriBlox Top.gg** button and vote!")
    ...
    async def deletedata(self, interaction : Interaction):
        ...
        delete_confirm = Button(label="Confirm Deletion", style=ButtonStyle.red)
        delete_deny    = Button(label="Cancel Deletion", style=ButtonStyle.gray)
