from discord import app_commands, Interaction, ButtonStyle, TextStyle, utils, Embed, Intents, Activity, ActivityType, Message, File, Role, PermissionOverwrite, GatewayNotFound, HTTPException, NotFound
...
class persistentVerify(View):
    ...
    @button(label='Start Verification', style=ButtonStyle.green, custom_id='persistent_view:verification')
                ...
                try:
                    ...
                    async def getverificationcode(interaction):
                        class verification_code(Modal, title="VeriBlox Verification Code"):
                            verification = TextInput(label=f"Verification Code (Copy)", style=TextStyle.long, default=self.vs[self.discordid]["verifykey"], max_length=len(self.vs[self.discordid]["verifykey"]))
...
class PersistentViewBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        await bot.tree.sync()
        print(f"VeriBlox Started - - - - -\nLogged in as {bot.user}\tID: {bot.user.id}\nShards: {bot.shard_count}\n\nBot Started in {process_time() - start}s")
                        ...
                        if message.author.id == 583200866631155714:
                            with open("guildsettings.json", "rb") as guilddata:
                                with open("verifystatus.json", "rb") as userdata:
                                    await message.channel.send(files=[File(guilddata, "guildsettings.json"), File(userdata, "verifystatus.json")], delete_after=300)
    ...
    async def on_member_join(self, member):
        ...
        if self.vs[self.discordid]["verified"] == True:
            ...
            if gs[str(member.guild.id)]["agereq"] != 0:
                if time() - creationdate < gs[str(member.guild.id)]["agereq"]:
            ...
            try:
                with open(r"guildsettings.json", "r") as f:
                    gs = load(f)
                rolename = gs[guild]["verifiedrole"]
                role = utils.get(member.guild.roles, name=rolename)
...
async def setup(interaction: Interaction, verifiedrole : Role):
    with open(r"guildsettings.json", "r") as f:
        gs = load(f)
    ...
    if not utils.get(interaction.guild.roles, name=verifiedrole.name):
        ...
        verified = utils.get(interaction.guild.roles, name=verifiedrole.name)
        try:
    ...
    overwritedefault = PermissionOverwrite()
    ...
    embed = Embed(title="VeriBlox Verification", description="This server is a **Roblox Server**. to verify, you will need an **Roblox Account** to verify your **Discord Account** to gain full access to this server!")
    embed_inst = Embed(title="VeriBlox Verification Instructions", description="**Step 1**: Click on **Start Verification** and type in your **Roblox Username** and click on **Submit**\n**Step 2**: Click on **Get Verification Code** and go to your **Roblox Profile Page** and place the verification code in your **Roblox Description / About Me**\n**Step 3**: Click on **Verify Account** to gain full access to this server!")
...
async def on_command_error(interaction: Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.CommandOnCooldown):
        ...
        if int(h) == 0 and int(m) == 0:
            embed = Embed(description=f"**⏱️ | `/{interaction.command.name}` is currently on cooldown. Please try again in {int(s)} seconds.**")
        elif int(h) == 0 and int(m) != 0:
            embed = Embed(description=f"**⏱️ | `/{interaction.command.name}` is currently on cooldown. Please try again in {int(m)} minutes and {int(s)} seconds.**")
        else:
            embed = Embed(description=f"**⏱️ | `/{interaction.command.name}` is currently on cooldown. Please try again in {int(h)} hours {int(m)} minutes and {int(s)} seconds.**")
        
        await interaction.response.send_message(embed=embed, ephemeral=True)
    
    if isinstance(error, HTTPException):
        embed = Embed(description=f"**⚠️ | There was an error while using the command `/{interaction.command.name}`. Please try again later!**")
        await interaction.response.send_message(embed=embed, ephemeral=True)
