    ...
    async def verification(self, interaction: discord.Interaction, button: ui.Button):
        class verificationModal(ui.Modal, title="VeriBlox Verification"):
            ...
            async def on_submit(self, interaction: discord.Interaction):
                robloxUser_bu = get(f"https://api.roblox.com/users/get-by-username?username={self.userName}").json()
                ...
                try:
                        ...
                        if vs[self.userID]["verifyCode"] == robloxUser["description"]:
                            ...
                            if interaction.guild.me.guild_permissions.manage_nicknames:
                                ...
                                if self.gs[self.guildID]["welcomemessage"] != "":
                                    await interaction.user.send(self.gs[self.guildID]["welcomemessage"])
                    ...
                    robloxUserName = robloxUser["username"]
                    robloxDisplayName = robloxUser["displayname"]
                    await interaction.response.send_message(content=f"**Account Found!** Verifying as **{robloxUserName} ({robloxDisplayName}**)\n> You can only regenerate your verification code once every **30 seconds**.", view=view, ephemeral=True)
...
class VeriBlox(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        await bot.load_extension("cogs.background.autoverify")
        await bot.load_extension("cogs.guild.config")
        await bot.load_extension("cogs.guild.server")
        await bot.load_extension("cogs.user.commands")
                ...
                try:
                    if msg.split()[1] == "reload":
                        if message.author.id == 583200866631155714:
                            ...
                            await bot.unload_extension("cogs.background.autoverify")
                            await bot.unload_extension("cogs.guild.config")
                            await bot.unload_extension("cogs.guild.server")
                            await bot.unload_extension("cogs.user.commands")
